...
func NewShortcutsMgr() *ShortcutsMgr {
	return &ShortcutsMgr{}
}

func (s *ShortcutsMgr) Create(id, command, binding string) error {
	...
	return s.setParams(id, command, binding)
}
...
func (s *ShortcutsMgr) setParams(id, command, binding string) error {
	path := s.getEntryPath(id)
	schema := fmt.Sprintf(
